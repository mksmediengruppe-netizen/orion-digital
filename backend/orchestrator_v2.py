...
Для сложных — вызов Opus/MiniMax для составления плана.
"""
...
AGENTS_CAPABILITIES = """
Доступные агенты:
1. DESIGNER (MiniMax) — HTML/CSS, лендинги, UI/UX, вёрстка, баннеры
2. DEVELOPER (MiniMax) — Python, Node.js, PHP, API, базы данных, боты, скрипты
3. DEVOPS (MiMo) — SSH, nginx, Docker, SSL, DNS, деплой, миграция серверов
4. INTEGRATOR (MiMo) — Битрикс24, Telegram, платежи, n8n, вебхуки, CRM
5. TESTER (MiMo) — тестирование сайтов, форм, SSL, мобильной версии
6. ANALYST (MiniMax/Sonnet) — архитектура, анализ, отчёты, code review
...
2. Designer для дизайна. В режиме standard/turbo — model: gemini_flash. В режиме pro — model: sonnet.
...
,
    "premium": """Ты — Claude Opus, главный архитектор ORION Digital.
...
MODEL_MAP = {
    ...
    "minimax":  "openai/gpt-5.4-mini",       # brain: думает, пишет код, дизайн
...
class Orchestrator:
    def __init__(self, call_llm_func, orion_mode="fast"):
...
def get_model_for_agent(agent_key, orion_mode="fast", task_hint=""):
    # ── SMART TURBO: Opus планирует, MiniMax кодит, MiMo деплоит ──
    if orion_mode == "standard":
        _ak = agent_key.lower().strip()
        # Orchestrator/planner → Opus (вызывается ОДИН РАЗ для плана)
        if _ak in ("orchestrator", "planner", "premium"):
            return MODEL_MAP["opus"]
        # HANDS agents (SSH, деплой) → MiMo
    ...
    if agent_key=="premium" or (agent_key=="analyst" and orion_mode=="premium"): return MODEL_MAP["opus"]
    if orion_mode=="premium" and agent_key in ("code_reviewer","intent_clarifier","orchestrator"): return MODEL_MAP["opus"]
    ...
    return MODEL_MAP.get("minimax", "openai/gpt-5.4-mini")  # PATCH fix: minimax as default for pro modes
