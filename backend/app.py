...
_p6_logger = _p6_logging.getLogger("phase6")

# Re-alias for Phase 6 code
try:
    logger
except NameError:
    logger = _p6_logger
...
import time as _phase6_time
app.config["START_TIME"] = _phase6_time.time()

# ── Monitoring ──
try:
    from monitoring import metrics_middleware, register_metrics_routes
    metrics_middleware(app)
    register_metrics_routes(app)
    logger.info("[PHASE6] Monitoring integrated")
except Exception as e:
    logger.warning(f"[PHASE6] Monitoring failed: {e}")

# ── Rate Limiting ──
try:
    from rate_limiter import rate_limit_middleware, register_rate_limit_routes
    rate_limit_middleware(app)
    register_rate_limit_routes(app)
    logger.info("[PHASE6] Rate limiting integrated")
except Exception as e:
    logger.warning(f"[PHASE6] Rate limiting failed: {e}")

# ── Task Scheduler ──
try:
    from task_scheduler import register_scheduler_routes, get_scheduler
    register_scheduler_routes(app)
    logger.info("[PHASE6] Task scheduler integrated")
except Exception as e:
    logger.warning(f"[PHASE6] Task scheduler failed: {e}")

# ── Image Generation ──
try:
    from image_generator import register_image_routes
    register_image_routes(app)
    logger.info("[PHASE6] Image generation integrated")
except Exception as e:
    logger.warning(f"[PHASE6] Image generation failed: {e}")

# ── Presentation Generation ──
try:
    from presentation_generator import register_presentation_routes
    register_presentation_routes(app)
    logger.info("[PHASE6] Presentation generation integrated")
except Exception as e:
    logger.warning(f"[PHASE6] Presentation generation failed: {e}")

# ── File Upload ──
try:
    from file_upload import register_upload_routes
    register_upload_routes(app)
    logger.info("[PHASE6] File upload integrated")
except Exception as e:
    logger.warning(f"[PHASE6] File upload failed: {e}")

# ── Skills System ──
try:
    from skills_system import register_skills_routes
    register_skills_routes(app)
    logger.info("[PHASE6] Skills system integrated")
except Exception as e:
    logger.warning(f"[PHASE6] Skills system failed: {e}")

# ── Backup Manager ──
try:
    from backup_manager import register_backup_routes
    register_backup_routes(app)
    logger.info("[PHASE6] Backup manager integrated")
except Exception as e:
    logger.warning(f"[PHASE6] Backup manager failed: {e}")

logger.info("[PHASE6] All modules integrated successfully!")
...
# ══════════════════════════════════════════════════════════════════
# ██ ORION SPRINT 5 — NEW API ENDPOINTS ██
# ══════════════════════════════════════════════════════════════════
...
@app.route("/api/cost/session", methods=["GET"])
@require_auth
...
@app.route("/api/agent-zones", methods=["GET"])
def get_agent_zones():
...
try:
    ...
    logging.info("[APP] LLM Cache initialized")

    @app.route("/api/cache/stats", methods=["GET"])
    @require_auth
    def get_cache_stats():
        """Return LLM cache statistics."""
        return jsonify({"success": True, "stats": _llm_cache.stats()})

    @app.route("/api/cache/invalidate", methods=["POST"])
    @require_admin
    def invalidate_cache():
        """Invalidate LLM cache (admin only)."""
        model = (request.get_json() or {}).get("model")
        count = _llm_cache.invalidate(model=model)
        return jsonify({"success": True, "invalidated": count})

except ImportError as e:
    logging.warning(f"[APP] LLM Cache not available: {e}")
except Exception as e:
    logging.warning(f"[APP] LLM Cache init failed: {e}")
...
try:
    from tts_tool import tool_text_to_speech, TTS_TOOL_SCHEMA
    logging.info("[APP] TTS tool loaded")
    ...
    def api_text_to_speech():
        ...
        if result.get("success"):
            # Return file URL instead of local path
            filename = result.get("filename", "")
            result["url"] = f"/files/tts/{filename}" if filename else None
            return jsonify({"success": True, **result})
        else:
            return jsonify(result), 500

except ImportError as e:
    logging.warning(f"[APP] TTS tool not available: {e}")
except Exception as e:
    logging.warning(f"[APP] TTS init failed: {e}")


# ══════════════════════════════════════════════════════════════════
# ██ EPHEMERAL DOCKER SANDBOXES — Per-task isolation ██
# ══════════════════════════════════════════════════════════════════

try:
    from ephemeral_sandbox import get_sandbox_manager, EPHEMERAL_SANDBOX_TOOLS
    _sandbox_mgr = get_sandbox_manager()
    logging.info("[APP] Ephemeral Sandbox Manager initialized")

    @app.route("/api/sandboxes", methods=["GET"])
    @require_auth
    def list_sandboxes():
        """List all active ephemeral sandboxes."""
        return jsonify({"success": True, "sandboxes": _sandbox_mgr.list_sandboxes()})
    ...
    def create_sandbox():
        ...
        data = request.get_json() or {}
        result = _sandbox_mgr.create_sandbox(
    ...
    def destroy_sandbox(sandbox_id):
        ...
        result = _sandbox_mgr.destroy_sandbox(sandbox_id)
        return jsonify(result)

    @app.route("/api/sandboxes/<sandbox_id>/exec", methods=["POST"])
    @require_auth
    def exec_in_sandbox(sandbox_id):
        """Execute command in a sandbox."""
        data = request.get_json() or {}
        command = data.get("command", "")
        if not command:
            return jsonify({"success": False, "error": "command required"}), 400
        result = _sandbox_mgr.exec_command(sandbox_id, command,
                                           timeout=data.get("timeout", 30))
        return jsonify(result)

except ImportError as e:
    logging.warning(f"[APP] Ephemeral Sandbox not available: {e}")
except Exception as e:
    logging.warning(f"[APP] Ephemeral Sandbox init failed: {e}")


# ══════════════════════════════════════════════════════════════════
# ██ VISUAL BROWSER AUTOMATION — Bounding boxes & element tagging ██
# ══════════════════════════════════════════════════════════════════

try:
    from visual_browser import VisualBrowser, VISUAL_BROWSER_TOOLS
    logging.info("[APP] Visual Browser tools loaded")

    @app.route("/api/browser/navigate", methods=["POST"])
    @require_auth
    def api_browser_navigate():
        """Navigate browser to URL with visual element tagging."""
        data = request.get_json() or {}
        url = data.get("url", "")
        if not url:
            return jsonify({"success": False, "error": "url required"}), 400
        vb = VisualBrowser()
        result = vb.navigate(url, focus=data.get("focus"))
        # Don't send screenshot in JSON (too large), just elements
        result.pop("screenshot_base64", None)
        return jsonify(result)

except ImportError as e:
    logging.warning(f"[APP] Visual Browser not available: {e}")
except Exception as e:
    logging.warning(f"[APP] Visual Browser init failed: {e}")


# ══════════════════════════════════════════════════════════════════
# ██ SURGICAL FILE EDITING — Diff-based find/replace ██
# ══════════════════════════════════════════════════════════════════

try:
    from surgical_file_edit import (
        tool_file_edit, tool_file_read, tool_file_write, tool_file_append,
        SURGICAL_FILE_TOOLS
    )
    logging.info("[APP] Surgical File Edit tools loaded")

    @app.route("/api/files/edit", methods=["POST"])
    @require_auth
    def api_file_edit():
        """Surgical file editing with find/replace."""
        data = request.get_json() or {}
        path = data.get("path", "")
        edits = data.get("edits", [])
        if not path or not edits:
            return jsonify({"success": False, "error": "path and edits required"}), 400
        result = tool_file_edit(path, edits, sandbox_id=data.get("sandbox_id"))
        return jsonify(result)

except ImportError as e:
    logging.warning(f"[APP] Surgical File Edit not available: {e}")
except Exception as e:
    logging.warning(f"[APP] Surgical File Edit init failed: {e}")
...
@app.route("/api/chats/<chat_id>/model", methods=["PUT"])
@require_auth
def change_chat_model(chat_id):
    """Change model for a chat. Returns notification that model will apply from next message."""
    db = db_read()
    chat = db["chats"].get(chat_id)
    if not chat:
        return jsonify({"error": "Chat not found"}), 404
    ...
    data = request.get_json() or {}
    new_mode = data.get("mode") or data.get("variant") or data.get("model")
    if not new_mode:
        return jsonify({"error": "mode/variant/model required"}), 400

    old_mode = chat.get("orion_mode", "fast")
    chat["orion_mode"] = new_mode
    db_write(db)

    # Return Manus-style notification
    return jsonify({
        "success": True,
        "notification": {
            "type": "model_change",
            "message": f"Модель изменена на {new_mode}. Будет применена со следующего сообщения.",
            "old_mode": old_mode,
            "new_mode": new_mode,
            "chat_id": chat_id,
        }
...
# ══════════════════════════════════════════════════════════════════
# ██ PARALLEL TASKS STATUS — Active tasks across all chats ██
# ══════════════════════════════════════════════════════════════════

@app.route("/api/tasks/active", methods=["GET"])
@require_auth
def get_active_tasks_list():
    """List all currently running tasks across chats."""
    try:
        from shared import _running_tasks, _tasks_lock
        with _tasks_lock:
            active = []
            for chat_id, task in _running_tasks.items():
        ...
        return jsonify({"success": True, "active_tasks": active, "count": len(active)})
    except Exception as e:
        return jsonify({"success": True, "active_tasks": [], "count": 0})
