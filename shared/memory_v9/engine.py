...
class SuperMemoryEngine:
    ...
    def get_context_for_task(self, user_message: str) -> str:
        """
        FIX #4: Return memory context string for injection into system prompt.
        Called by AgentLoop._get_memory_context().
        """
        try:
            # Try episodic memory first
            if hasattr(self, '_episodic') and self._episodic:
                memories = self._episodic.search(user_message, limit=3)
                if memories:
                    lines = []
                    for m in memories:
                    ...
                    if lines:
                        return "Из памяти предыдущих задач:\n" + "\n".join(lines)
        except Exception:
            pass
        return ""
