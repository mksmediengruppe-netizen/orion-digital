...
function mapApiUser(apiUser: any): CurrentUser {
  const rawRole = apiUser.role || "user";
  const normalizedRole: UserRole = rawRole === "super_admin" ? "admin" : (["admin", "manager", "user", "viewer"].includes(rawRole) ? rawRole as UserRole : "user");
  return {
    id: apiUser.id,
