...
function TasksSection({ searchQuery, setSearchQuery }: { searchQuery: string; setSearchQuery: (v: string) => void }) {
  ...
  React.useEffect(() => {
    fetch("/api/admin/tasks?limit=500")
      .then(r => r.json())
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{loading ? "Загрузка..." : `${filtered.length} задач`}</p>
        </div>
        <div className="relative">
          <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
          <input
      ...
      <div className="bg-white rounded-xl border border-[#E8E6E1] overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#E8E6E1] bg-[#F8F7F5]">
              {["Задача", "Проект", "Статус", "Модель", "Время", "Стоимость", "Дата"].map(h => (
                <th key={h} className="text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider px-4 py-2.5">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-400">Загрузка задач...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-400">Задачи не найдены</td></tr>
            ) : filtered.map(task => (
              <tr key={task.run_id || task.id} className="border-b border-[#E8E6E1] last:border-0 hover:bg-[#F8F7F5] transition-colors">
                <td className="px-4 py-3 text-sm font-medium text-gray-800 max-w-[200px] truncate" title={task.name || task.title}>{task.name || task.title || "-"}</td>
                <td className="px-4 py-3 text-xs text-indigo-600 font-mono">{task.project_name || task.project_id || "-"}</td>
                <td className="px-4 py-3"><StatusBadge status={task.status as any} size="sm" /></td>
                <td className="px-4 py-3 text-xs text-gray-500">{task.model || "Arcane"}</td>
                <td className="px-4 py-3 text-xs font-mono text-gray-600">{task.duration || "-"}</td>
                <td className="px-4 py-3 text-sm font-mono text-gray-700">${(task.cost || 0).toFixed(4)}</td>
                <td className="px-4 py-3 text-xs text-gray-400">{formatDate(task.createdAt)}</td>
              </tr>
            ))}
          </tbody>
        </table>
