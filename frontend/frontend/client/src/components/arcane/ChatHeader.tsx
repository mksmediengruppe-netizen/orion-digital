// ARCANE ChatHeader — model selector + status + timer + controls
// Layout: [breadcrumb] ... [$cost] [timer] [model▾] [status] [panel] [⋯]

import { cn } from "@/lib/utils";
import { type AgentStatus } from "@/lib/mockData";
import { PanelRight, MoreHorizontal, DollarSign, Clock, Square, Zap, Gauge, Sparkles, Crown, ChevronDown, Check, AlertTriangle, Terminal, Keyboard, Image, Palette, ShieldCheck } from "lucide-react";
import {
...
export type ModelKey = "mini" | "standard" | "sonnet" | "premium";
...
export const MODELS: Record<ModelKey, ModelDef> = {
  ...
  premium: {
    ...
    model: "Claude Sonnet 4.6",
    pricePerTask: "~$0.50-1.00",
    budget: "Лимит $25",
    speed: "👑 Максимум",
    desc: "Лучшая модель + фото + проверка дизайна и кода",
  },
};
...
interface ChatHeaderProps {
  ...
  onModelOptionsChange?: (opts: ModelOptions) => void;
...
export function ChatHeader({
  ...
  model = "standard",
  onModelChange,
  modelOptions = { premiumImages: false, designCheck: false, premiumReview: false },
  ...
  // When switching to premium, auto-enable all toggles
  const handleModelChange = (key: ModelKey) => {
    onModelChange?.(key);
    if (key === "premium") {
      onModelOptionsChange?.({ premiumImages: true, designCheck: true, premiumReview: true });
    }
    setModelOpen(false);
  };

  const updateOption = (field: keyof ModelOptions, value: boolean) => {
    onModelOptionsChange?.({ ...modelOptions, [field]: value });
  };
    ...
    <header className="h-12 border-b border-[#E8E6E1] dark:border-[#2a2d3a] bg-white dark:bg-[#0f1117] flex items-center px-4 gap-2 shrink-0">
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <DropdownMenu open={modelOpen} onOpenChange={setModelOpen}>
          ...
          <DropdownMenuContent align="end" className="w-80 p-1.5" sideOffset={6}>
            ...
            {(Object.entries(MODELS) as [ModelKey, ModelDef][]).map(([key, m]) => (
              <button
                ...
                className={cn(
                  ...
                  model === key ? "bg-gray-50 dark:bg-gray-800" : "hover:bg-gray-50 dark:hover:bg-gray-800"
                )}
              >
                <div className={cn(
            ...
            <div className="px-2 py-1 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Дополнительные опции</div>

            {/* Toggle 1: Premium Images */}
            <div className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium text-gray-700 dark:text-gray-300">Премиум изображения</div>
                <div className="text-[10px] text-gray-400">Фотореалистичные AI-картинки · +$0.10-0.50</div>
              </div>
              <ToggleSwitch checked={modelOptions.premiumImages} onChange={(v) => updateOption("premiumImages", v)} />
            </div>

            {/* Toggle 2: Design Check */}
            <div className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium text-gray-700 dark:text-gray-300">Проверка дизайна</div>
                <div className="text-[10px] text-gray-400">AI оценит качество дизайна · +$0.03</div>
              </div>
              <ToggleSwitch checked={modelOptions.designCheck} onChange={(v) => updateOption("designCheck", v)} />
            </div>

            {/* Toggle 3: Premium Review */}
            <div className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              <div className="w-6 h-6 rounded-md bg-emerald-50 flex items-center justify-center shrink-0">
                <ShieldCheck size={12} className="text-emerald-500" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium text-gray-700 dark:text-gray-300">Премиум проверка</div>
                <div className="text-[10px] text-gray-400">Второй AI проверит результат · +$0.05</div>
              </div>
              <ToggleSwitch checked={modelOptions.premiumReview} onChange={(v) => updateOption("premiumReview", v)} />
            </div>

            <DropdownMenuSeparator className="my-1" />
            <div className="px-3 py-1.5 text-[10px] text-gray-400">
              Текущий выбор: <span className="font-medium text-gray-600 dark:text-gray-300">{current.model}</span> · {current.speed}
            </div>
          </DropdownMenuContent>
        </DropdownMenu>
