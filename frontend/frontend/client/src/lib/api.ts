...
const TOKEN_KEY = "arcane_session_token";
...
const auth = {
  ...
  login: async (email: string, password: string) => {
    const resp = await post<LoginResponse>("/api/auth/login", { login_id: email, password });
...
const chats = {
  ...
  status: (chatId: string) =>
    get<{ chat_id: string; status: string; total_cost: number; title: string; last_message: string | null; message_count: number }>(`/api/chats/${chatId}/status`),
};
    ...
    if (raw.chat_models) {
      Object.entries(raw.chat_models).forEach(([id, m]) => {
        if (!result.find(r => r.id === id)) {
          result.push({ id, name: m.name, provider: "arcane", available: true });
        }
      });
    }
...
export type TaskMode = "top" | "optimum" | "lite" | "free";
...
const projects = {
  list: () => get<{ projects: ProjectState[] }>("/api/projects"),

  create: (name: string, description?: string, mode?: TaskMode, budget_limit?: number) =>
    post<{ ok: boolean; project: ProjectState; path: string }>("/api/projects", {
      name,
      description,
      mode,
      budget_limit,
  ...
  submitTask: (projectId: string, task: string, mode?: TaskMode, budget_limit?: number) =>
    post<{ ok: boolean; run: RunResult }>(`/api/projects/${projectId}/tasks`, {
      task,
      mode,
      budget_limit,
    }),
  ...
  getRun: (runId: string) =>
    get<{ run: RunResult }>(`/api/runs/${runId}`),

  cancelRun: (runId: string) =>
    post<{ ok: boolean }>(`/api/runs/${runId}/cancel`, {}),
  ...
  connectWS: (
    ...
    const ws = new WebSocket(`${proto}://${host}/ws/${projectId}`);
    ws.onmessage = (e) => {
      try {
