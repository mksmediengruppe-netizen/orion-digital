...
import { type Step, type Message, type AgentStatus, type Chat, type Project, type ViewerArtifact } from "@/lib/mockData";
...
import OrchestratorProgress from "@/components/arcane/OrchestratorProgress";
...
export default function Home() {
  ...
  // Projects: fetched from real API
    ...
    fetch("/api/projects", { credentials: "include" })
      ...
      .then(data => {
        if (data?.projects?.length > 0) {
          ...
          // Fallback: single default project
          setProjects([{ id: "default", name: "Мой проект", description: "" }]);
        }
      })
      .catch(() => setProjects([{ id: "default", name: "Мой проект", description: "" }]));
  }, []);
  // Real chats from API; fall back to empty array while loading
  const allChats = chatsAPI.chats;  // real API via useChatsAPI
  const setAllChats = (_fn: ((prev: Chat[]) => Chat[]) | Chat[]) => {}; // no-op: managed by chatsAPI

  const { theme, toggleTheme } = useTheme();

  // ─── Orchestrator task mode ───────────────────────────────────────────────
  const [inputMode, setInputMode] = useState<"chat" | "task">("chat");
  const projectTask = useProjectTask();

  const handleSubmitTask = useCallback(async (text: string, mode: TaskMode) => {
    await projectTask.submitTask(text, mode);
    toast.info(`Задача отправлена в оркестратор (${mode.toUpperCase()})`);
  }, [projectTask]);
  ...
  const chat = allChats.find(c => c.id === activeChat) ?? allChats[0];
  ...
  const chatStatus = realAgentStatus || (chat?.status ?? "idle");
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-[#0f1117] border-x border-[#E8E6E1] dark:border-[#2a2d3a]">
                ...
                >
                  ...
                  {activeChat !== "c1" && isRunning && (headerStatus === "thinking" || headerStatus === "executing" || headerStatus === "searching") && (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 px-4 py-3 bg-white border border-[#E8E6E1] rounded-2xl rounded-tl-sm shadow-sm">
                        <StatusBadge status={chat?.status ?? "idle"} size="sm" />
                        <div className="flex gap-1">
                          {[0, 1, 2].map(i => (
                            <motion.span
                              key={i}
          ...
          {/* Orchestrator progress panel (task mode) */}
          {inputMode === "task" && projectTask.progress && (
            <div className="px-4 pb-2">
              <OrchestratorProgress
                progress={projectTask.progress}
                onCancel={projectTask.cancelTask}
                className="max-h-64 overflow-y-auto"
              />
            </div>
          )}

          {/* Mini preview above composer — shows current agent activity (chat mode) */}
          {inputMode === "chat" && isRunning && (
            <MiniPreview
          ...
          <Composer
            ...
            budgetExhausted={budgetExhausted}
            onAdminRefill={handleAdminRefill}
            isRunning={inputMode === "task" ? projectTask.isRunning : isRunning}
            onStop={inputMode === "task" ? projectTask.cancelTask : undefined}
          />
        </div>
