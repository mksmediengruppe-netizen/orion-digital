// ARCANE useSSEConnection — real EventSource hook with automatic reconnect
// Features:
...
import type { SSEConnectionState } from "@/components/arcane/SSEReconnectingBanner";
...
export function useSSEConnection({
  ...
  const connect = useCallback(() => {
    ...
    const namedEvents = ["agent_status", "step_update", "task_complete", "cost_update", "notification", "auth_required", "browser_takeover", "research_step", "tool_progress"];
...
// In the ARCANE prototype there is no real SSE backend, so this wrapper
